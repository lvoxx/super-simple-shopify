...
- [x] `identity`: StaffUser + Role/RBAC model, `IdentityFacade`, `StaffInvited` (tenant-sharded). _(Session/ApiToken/login/token-issuance are intentionally omitted — auth is delegated to Keycloak at the gateway; modules are not resource servers. See ADR 0004.)_
- [x] Tenant resolution: host/domain → shop for storefront; auth → shop for admin. _(`TenantBindingFilter`: `X-Shop-Id` for admin, `HostTenantResolver` SPI over `StoreFacade.findByDomain` for storefront.)_
- [x] Shard assignment on shop creation (shop → shard mapping table on a control plane). _(See ADR 0003.)_
- [x] Admin auth wired through `platform-security`; `TenantContext` populated per request. _(`Principal` built from `X-Auth-Subject`/`X-Auth-Roles` and bound to `SecurityContext` alongside `TenantContext`.)_
